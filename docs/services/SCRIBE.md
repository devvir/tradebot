...
- **Boundary hold (exactly-once):** the trailing run of rows at the stream's current max `tsField` instant is held back rather than emitted; a strictly newer instant (or exhaustion) flushes it. `startTime` is inclusive, so when a block ends *on* that instant the reanchored window re-delivers every row of it from offset 0 — the held copies are dropped and the fresh ones re-buffered, so boundary rows come out exactly once instead of duplicated at every block seam. When the `+1ms` safeguard skips past the instant instead, the run is flushed (the next window won't re-deliver it). Exactly-once holds as long as one instant doesn't span more rows than a whole block can fetch; past that, rows of the instant beyond the offset cap are unreachable and dropped by the `+1ms` skip — a pre-existing, bounded limitation the hold neither fixes nor worsens.
