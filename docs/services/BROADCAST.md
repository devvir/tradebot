...
For each requested non-`default` pool, the **fannable** channels are subscribed with the empty-symbol form `<channel>::<Pool>` (e.g. `orderBookL2::Primary`), which pool-filters every symbol in one subscription, **on that pool's own socket** (the connection key includes the pool). `default` keeps the bare subscription on the no-pool socket. Requesting `primary,secondary` opens a Primary socket and a Secondary socket, each subscribing the fannable channels once; both pools' messages flow to the same exchange/routing key. Each row already carries its own `pool` field, so downstream reads the pool straight from the row content — nothing pool-specific is added to the message headers. The per-pool socket is required because BitMEX keys a subscription by its bare topic and rejects the same topic twice on one connection.

A channel is fannable when its **default** (bare) subscription streams the fused `Aggregated` pool, so per-pool data can only be obtained by subscribing per pool. The fannable set is `POOL_FANOUT_CHANNELS` (`@tradebot/utils`) — the order books (`orderBookL2`, `orderBookL2_25`, `orderBook10`) and the bins — and `fanByPool` (`src/pools.ts`) is membership in it. `trade` and `quote` are **not** fanned: their bare stream already tags every row `Primary`/`Secondary` (never `Aggregated`), so both pools arrive on the no-pool socket, per-row tagged. `instrument` carries no `pool` at all. For a fannable channel `default` and `aggregated` yield the same rows (bare vs explicit `::Aggregated`).
