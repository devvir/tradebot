...
**Pool as a column.** Liquidity pool is an ordinary column: the pooled tables (`orderBookL2`, `trade`, `quote`) carry a `pool` field, distinguished by that column. `trade` and `quote` store every pool in the one file. `orderBookL2` is the exception — its two pools arrive on two separate WS subscriptions (different filters on the same channel) whose messages are not co-temporal, so they cannot share a file; the Secondary pool lands in the `orderBookL2.secondary` pseudo-table (below). `tick` and `instrument` follow their own rules (`tick` is unpooled; `instrument`'s `pool` column exists but the feed leaves it empty).

**Pseudo-tables.** A dotted `<table>` (e.g. `<table>.<variant>`) is a *pseudo-table* of the undotted real one — a variant slice stored in its own directory but sharing the real table's columns and its BitMEX identity. `baseTable` (`data/headers.ts`) strips the suffix so every table-identity lookup resolves `<table>.<anything>` to `<table>`: header and cast resolution (`headersFor`/`applyCasts`) and the read-path decode strategy (`WS_TABLES`/`FREE_TEXT_TABLES` membership in `data/decode.ts`/`data/parse.ts`) — so a WS pseudo-table like `orderBookL2.secondary` decodes into message envelopes, not plain rows, exactly like its base. This is identity-level only: storage stays separate, and no per-variant entry is ever added to `TABLE_HEADERS`/`TABLE_CASTS`. It is a general variant facility — `orderBookL2.secondary` is its one current use — and one rule covers any future variant. (This table-name suffix is independent of the per-file `<date>.<suffix>` filename tag below.)
