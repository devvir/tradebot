...
**Pool as a column.** Liquidity pool is an ordinary column, not a table split: the pooled tables (`orderBookL2`, `trade`, `quote`) carry a `pool` field and store every pool in the one table, distinguished by that column. `tick` and `instrument` follow their own rules (`tick` is unpooled; `instrument`'s `pool` column exists but the feed leaves it empty).

**Pseudo-tables.** A dotted `<table>` (e.g. `<table>.<variant>`) is a *pseudo-table* of the undotted real one — a variant slice stored in its own directory but sharing the real table's columns. `baseTable` (`data/headers.ts`) strips the suffix for every header and cast lookup, so `headersFor`/`applyCasts` resolve `<table>.<anything>` to `<table>`. This is column-level only: storage stays separate, and no per-variant entry is ever added to `TABLE_HEADERS`/`TABLE_CASTS`. It is a general variant facility (not used for pools, which are a column); one rule covers any future variant. (This table-name suffix is independent of the per-file `<date>.<suffix>` filename tag below.)
