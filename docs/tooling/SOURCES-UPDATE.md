...
`sources update` scans the vault, identifies files that exist but haven't completed their pipeline journey, and offers to move them along. It is task-oriented: if a file is simply absent it's ignored — only existing files that should have been copied, moved, or prepared but weren't produce tasks.

**Read-only inspection** lives in `sources status`. Both commands share the same scanner. See [SOURCES-STATUS.md](SOURCES-STATUS.md) for vocabulary, vault layout, env vars, scanner mechanics, and state data model.
...
Collect every WS day that will have local source files (after pull) but has no bucket anywhere — no finalised bucket in local or Mega, and no `.csv.gz.tmp` indicating a bucket is currently being written (e.g. by a concurrent `sources prepare`). Groups by `(table, year)` for the summary display; execution is whole-vault (see Execution).
...
After scanning, `sources update` prints a summary before asking to execute anything. The summary is a bullet list: one line per task type that has work, nothing for task types with nothing to do.
...
```
src/tools/sources/
  scan/            shared scanner (see SOURCES-STATUS.md)

  update/
    run.ts         runUpdate() — entry point: find rsync temps → scan → derive → summarise → execute
...
**Cross-subcommand rule:** `update/` never imports from `status/`. Shared code lives at `sources/` or `sources/scan/`.
...
Failed operations are reported and counted but do not abort the run. Re-running `sources update` is always safe — tasks are re-derived from the current state, so any file that failed to transfer will appear again. No explicit retry within a session. This includes upload-verification failures and remote-cleanup size mismatches — both are counted as failures and re-surface on the next run.
