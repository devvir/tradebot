...
type Symbols = { instruments: string[]; indices: string[]; inactive: Set<string> };
...
export const fetchSymbols = async (baseUrl: string): Promise<Symbols> => {
  ...
  const instruments = all.filter(i => ! i.symbol.startsWith('.')).map(i => i.symbol);
  const inactive = new Set(all.filter(i => i.state !== 'Open').map(i => i.symbol));

  logger.info({ indices: indices.length, instruments: instruments.length, inactive: inactive.size }, 'Symbol list loaded');

  return { indices, instruments, inactive };
};
