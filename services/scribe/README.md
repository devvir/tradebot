...
- Collects OHLCV bins at all four BitMEX resolutions, all symbols in one stream: `tradeBin1m`/`5m`/`1h`/`1d` and `quoteBin1m`/`5m`/`1h`/`1d`, over the full history back to 2014-11-22
- Each table's behaviour is driven entirely by its entry in [settings.ts](src/utils/settings.ts) — a server-side `filter`, static query `params`, an optional `symbols` resolver (per-symbol subtasks), an optional `from` start floor — so the runner stays generic and names no table
...
| Env var | Description |
|---|---|
| `SCRIBE_TABLES` | Comma-separated table names to collect. Default empty = all of them. Use it to bring the bin tables up gradually — they share one rate budget with everything else. |
