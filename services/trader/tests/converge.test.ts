/**
 * Converge algorithm tests — ported from services/executor/tests/converge.test.ts.
 *
 * Key differences from the original:
 *   - desired uses OrderPlan (requires symbol + ordType)
 *   - live uses Order (leavesQty is optional, no cumQty field)
 *   - ordStatus 'Cancelled' (double-L, matching the trader Order type)
 */

import { describe, it, expect } from 'vitest';
import { converge, filterActiveOrders } from '../src/executor/converge';
import type { Order } from '../src/types';
import type { OrderPlan } from '../src/planner/types';

const THRESHOLD = 0.25; // absolute USD

function makeLive(overrides: Partial<Order> = {}): Order {
  return {
    orderID:   'oid-1',
...
function makeDesired(overrides: Partial<OrderPlan> = {}): OrderPlan {
  return {
    symbol:   'XBTUSD',
    side:     'Buy',
    ordType:  'Limit',
...
describe('converge — amend', () => {
  ...
  it('sets leavesQty to desired orderQty regardless of partial fill', () => {
    // leavesQty = 70 means 30 units were filled; desired wants 100 again → amend
    const desired = [makeDesired({ orderQty: 100 })];
    const live    = [makeLive({   leavesQty: 70, orderQty: 100 })];
    const result  = converge(desired, live, THRESHOLD);
...
describe('converge — positional matching', () => {
  it('matches bids closest-to-mid first (highest price first)', () => {
    ...
    // Live provided in reverse price order; qty matches only the correct pairing.
...
describe('filterActiveOrders', () => {
  const orders: Order[] = [
    ...
    makeLive({ orderID: 'd', clOrdID: 'tb_XBTUSD_4', symbol: 'XBTUSD', ordStatus: 'Cancelled' }),
  ...
  it('excludes orders not owned by this trader (different prefix)', () => {
    const result = filterActiveOrders(orders, 'XBTUSD');
