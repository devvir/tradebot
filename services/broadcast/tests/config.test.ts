...
describe('Configuration', () => {
  ...
  beforeEach(() => {
    process.env = {
      ...
      BROADCAST_MESSAGE_TTL: '1800000',
      BROADCAST_RECONNECT_DELAY_MS: '5000',
      BROADCAST_MAX_RECONNECT_DELAY_MS: '60000',
    };
  });
  ...
  describe('Connection settings', () => {
    it('should parse reconnect delay from env', () => {
      process.env.BROADCAST_RECONNECT_DELAY_MS = '10000';
      const config = loadConfig();
      expect(config.connection.reconnectDelayMs).toBe(10000);
    });

    it('should parse max reconnect delay from env', () => {
      process.env.BROADCAST_MAX_RECONNECT_DELAY_MS = '120000';
      const config = loadConfig();
  ...
  describe('RabbitMQ settings', () => {
    ...
    it('should parse message TTL from env', () => {
      process.env.BROADCAST_MESSAGE_TTL = '3600000';
      const config = loadConfig();
